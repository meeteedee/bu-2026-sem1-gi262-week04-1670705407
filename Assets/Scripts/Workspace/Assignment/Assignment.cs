...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS03_CheckValidBrackets()
        {
            ...
            {
                if (brackets.ContainsValue(c)) // ถ้าเป็นวงเล็บเปิด
                {
                    stack.AddLast(c);
                }
                else if (brackets.ContainsKey(c)) // ถ้าเป็นวงเล็บปิด
                {
                    if (stack.Count == 0 || stack.Last.Value != brackets[c])
                    {
                        isValid = false;
                        break;
                    }
        ...
        public void AS07_RemoveDuplicatesFromLinkedList()
        {
            ...
            if (list.Count <= 1)
            {
                foreach (int val in list) Debug.Log(val);
                return;
            }
